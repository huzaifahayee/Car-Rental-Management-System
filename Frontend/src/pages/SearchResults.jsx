            ...
            {!loading && !error && filtered.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                {filtered.map(vehicle => (
                  <VehicleCard key={vehicle.id} vehicle={vehicle} onBook={() => handleBook(vehicle.id)} />
                ))}
              </div>
            )}
          </main>
...
function VehicleCard({ vehicle, onBook }) {
  ...
  return (
    <article style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,.07)', opacity: available ? 1 : 0.7 }}>
      ...
      <div style={{ padding: '16px 18px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 }}>
          {[vehicle.hasAC && 'AC', vehicle.driverOption ? 'With Driver' : 'Self Drive', available ? 'Available' : vehicle.status.replace('_', ' ')].filter(Boolean).map(tag => (
            <span key={tag} style={{ background: tag === 'Available' ? '#e8faf2' : '#f5f7fa', color: tag === 'Available' ? '#00a85a' : '#555', padding: '3px 9px', borderRadius: 20, fontSize: 11, fontWeight: 700 }}>{tag}</span>
          ))}
        </div>
        <button disabled={!available} onClick={onBook} style={{ width: '100%', border: 'none', borderRadius: 10, padding: 10, background: available ? 'linear-gradient(90deg,#00c472,#00a85a)' : '#e5e7eb', color: available ? '#fff' : '#9ca3af', fontWeight: 800, cursor: available ? 'pointer' : 'not-allowed' }}>
          {available ? 'Book this vehicle' : 'Currently unavailable'}
        </button>
      </div>
    </article>
  )
